...
#define RT5621_VERSION "alsa 1.0.23 0.01"
...
//add hp detect status 
static int hp_dectect_flag = 0;
...
static struct rt5621_reg init_data[] =
{
    ...
    {RT5621_ADC_REC_GAIN,        	0xFA95},    //default stereo ADC gain to +15db
    ...
    {RT5621_MONO_AUX_OUT_VOL, 		0x8080},    //default Aux volume to 0db
    {RT5621_HP_OUT_VOL,             0x8484},    //default HP volume to -12db
    ...
    //{RT5621_GPIO_OUTPUT_PIN_CTRL,   0x0002},   
};
...
#if 0
static unsigned int power_reg[8];
static inline rt5621_power_reg_backup(struct snd_soc_codec *codec)
{
    //printk("***enter %s \n", __func__);
    if(power_reg[0] == 0)
    {
        memset(power_reg, 0, 8*sizeof(unsigned int));
           
        power_reg[1] = rt5621_read(codec, 0x3a);
        power_reg[2] = rt5621_read(codec, 0x3c);
        power_reg[3] = rt5621_read(codec, 0x3e);    
        power_reg[4] = rt5621_read(codec, 0x4a);
        power_reg[5] = rt5621_read(codec, 0x04);   
        power_reg[6] = rt5621_read(codec, 0x06);

        power_reg[0] = 1;  //used for backup flag
#if 0
        unsigned int i;
        for(i =0; i<8; i++)
        {
            printk("power_reg[%d] : %x \n", i, power_reg[i]);
        }
#endif
    }
}

static inline rt5621_power_reg_recovery(struct snd_soc_codec *codec)
{
    //printk("***enter %s and power_reg[0]: %x\n", __func__, power_reg[0]);
    if(power_reg[0] == 1)
    {
        rt5621_write(codec, 0x3a, power_reg[1]);
        rt5621_write(codec, 0x3c, power_reg[2]);
        rt5621_write(codec, 0x3e, power_reg[3]);
        rt5621_write(codec, 0x4a, power_reg[4]); 
        rt5621_write(codec, 0x04, power_reg[5]);
        rt5621_write(codec, 0x06, power_reg[6]);     
#if 0
        unsigned int i;
        for(i =0; i<8; i++)
        {
...
#endif
static ssize_t rt5621_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (!strcmp(attr->attr.name, "reg")) {        
        printk(KERN_INFO"----%s----\n", buf);
        sscanf(buf, "%02x", &reg_addr);
        if( ' ' == buf[2] ) {
			sscanf(buf+3, "%04x", &reg_data);
            printk(KERN_INFO"write reg 0x%02x 0x%04x\n", reg_addr, reg_data);
            rt5621_write(codec, reg_addr, reg_data);
        }
		else {            
            reg_data =rt5621_read(codec, reg_addr);
			printk(KERN_INFO"read reg 0x%02x 0x%04x\n",reg_addr, reg_data);				
		}
    }
...
static DEVICE_ATTR(reg, S_IRWXUGO, 0, rt5621_store);

static struct attribute *rt5621_attr[] = {
    &dev_attr_reg.attr,
    NULL
};

static struct attribute_group rt5621_attr_group = {
    .name = NULL,
    .attrs = rt5621_attr,
};
...
static int rt5621_ChangeCodecPowerStatus(struct snd_soc_codec *codec,int power_state)
{
    ...
    case POWER_STATE_D1_PLAYBACK:	//Low on of Playback
        ...
        rt5621_write_mask(codec,RT5621_PWR_MANAG_ADD1,PWR_MAIN_I2S_EN|PWR_HP_OUT_ENH_AMP|PWR_HP_OUT_AMP|PWR_AUX_OUT_AMP
                          ,PWR_MAIN_I2S_EN|PWR_HP_OUT_ENH_AMP|PWR_HP_OUT_AMP|PWR_AUX_OUT_AMP);
    ...
    case POWER_STATE_D2_PLAYBACK:	//STANDBY of playback
        ...
        rt5621_write_mask(codec,RT5621_PWR_MANAG_ADD3 ,0,/*PWR_HP_R_OUT_VOL|PWR_HP_L_OUT_VOL|*/PWR_SPK_OUT|PWR_AUXOUT_L_VOL_AMP|PWR_AUXOUT_R_VOL_AMP);

        rt5621_write_mask(codec,RT5621_PWR_MANAG_ADD1 ,0,PWR_HP_OUT_ENH_AMP|PWR_HP_OUT_AMP||PWR_AUX_OUT_AMP|PWR_AUX_OUT_ENH_AMP);
...
//flove112510_S
void switch_audio(int hpPlugedin)
{
    struct snd_soc_device *socdev = rt5621_socdev;
    struct snd_soc_codec *codec = socdev ->card->codec;

    hp_dectect_flag = hpPlugedin;
    if (hpPlugedin)
    {
        rt5621_AudioOutEnable(codec,RT_WAVOUT_HP,0);	//unmute hp out
        rt5621_write_mask(codec,RT5621_GPIO_OUTPUT_PIN_CTRL ,0x0000,0x0002);//Power off AMP
        rt5621_AudioOutEnable(codec,RT_WAVOUT_AUXOUT,1);	//mute auxout out
    }
    else
    {
        rt5621_AudioOutEnable(codec,RT_WAVOUT_HP,1);	//mute hp out
        rt5621_AudioOutEnable(codec,RT_WAVOUT_AUXOUT,0);	//unmute auxout out
        rt5621_write_mask(codec,RT5621_GPIO_OUTPUT_PIN_CTRL ,0x0002,0x0002);//Power on AMP
    }
    #if 0
    //add by benjamin.zhao
    if(power_reg[0] == 1)
    {
        memset(power_reg, 0, 8*sizeof(unsigned int));
        rt5621_power_reg_backup(codec);
    }
    #endif
}
//flove112510_E
...
void hp_depop_mode2(struct snd_soc_codec *codec)
{
    ...
    */
	...
	rt5621_write_mask(codec, 0x3a, 0x0100, 0x0100);
	rt5621_write_mask(codec, 0x3c, 0x2000, 0x2000);
	rt5621_write_mask(codec, 0x3e, 0x8600, 0x8600);
	rt5621_write_mask(codec, 0x5e, 0x0200, 0x0200);
	...
	rt5621_write_mask(codec, 0x3c, 0x3730, 0x3730);
	//output from hp mixer by default
	rt5621_write_mask(codec, 0x3a, 0x0120, 0x0120);
	rt5621_write_mask(codec, 0x3a, 0x0130, 0x0130);
	rt5621_write_mask(codec, 0x3a, 0x8130, 0x8130);
	//rt5621_write_mask(codec, 0x04, 0x4646, 0x4646);
	rt5621_write_mask(codec, 0x04, 0x0404, 0x0404);
	rt5621_write_mask(codec, 0x1c, 0x4740, 0x4740);
	rt5621_write_mask(codec, 0x5e, 0x0000, 0x0000);
    schedule_timeout_uninterruptible(msecs_to_jiffies(300));
#endif
}
                                 ...
                                 struct snd_soc_dai *codec_dai)
{
    ...
    //add by benjamin.zhao
    //rt5621_power_reg_recovery(codec);
    printk("enter %s and hp_dectect_flag: %d  \n", __func__, hp_dectect_flag);
    switch (stream)
    {
    case SNDRV_PCM_STREAM_PLAYBACK:
		
	//	hp_depop_mode2(codec);   //Roy 20110506 for test
                              ...
                              int pll_id,int source, unsigned int freq_in, unsigned int freq_out)
{
    ...
    printk("**enter %s, freq_in:%d , freq_out:%d, pll_id: %d\n", freq_in, freq_out, pll_id);
                                 ...
                                 int clk_id, unsigned int freq, int dir)
{
    ...
    if ((freq >= (256 * 8000)) && (freq <= (512 * 48000))) {
        pr_info("sysclk freq %u for audio i2s\n", freq);
        rt5621->sysclk = freq;
        return 0;
    }
    
    pr_alert("unsupported sysclk freq %u for audio i2s\n", freq);
    pr_alert("Set sysclk to 24.576Mhz by default\n");   
                                ...
                                struct snd_soc_dai *codec_dai)
{
    ...
    printk("rt5621_pcm_hw_params, rate:%d, coeff:%d, rt5621->sysclk:%d\n", params_rate(params), coeff, rt5621->sysclk);
    if (coeff < 0)
...
static void rt5621_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *codec_dai)
{
    ...
    case SNDRV_PCM_STREAM_PLAYBACK:
        ...
        rt5621_AudioOutEnable(codec,RT_WAVOUT_AUXOUT,0);	//mute auxout out
...
//#define RT5621_HIFI_RATES SNDRV_PCM_RATE_48000
#define RT5621_HIFI_RATES SNDRV_PCM_RATE_8000_48000 //realtek - 20110620
...
*/
    ...
    printk("***enter %s, and suspend_bias_level: %d \n", __func__, codec->suspend_bias_level);
...
static int rt5621_init(struct snd_soc_device *socdev)
{
    ...
    printk(KERN_INFO "alsa version is 1.0.18, codec driver version is 0.04\n");
    codec->name = "RT5621";
	...
	WRITE_CBUS_REG_BITS(PERIPHS_PIN_MUX_1, 1, 15, 1);
	WRITE_CBUS_REG_BITS(PERIPHS_PIN_MUX_1, 1, 19, 1);
      WRITE_CBUS_REG_BITS(PERIPHS_PIN_MUX_0, 1, 18, 1);
	
    audio_set_clk(AUDIO_CLK_FREQ_441, AUDIO_CLK_256FS);
    mdelay(10);
    rt5621_reset(codec);
    rt5621_write(codec, RT5621_PWR_MANAG_ADD3, 0x8000);//enable Main bias
    rt5621_write(codec, RT5621_PWR_MANAG_ADD2, 0x2000);//enable Vref
    rt5621_write(codec, RT5621_GPIO_PIN_CONFIG, 0x1C0C);//set GPIO to output
	rt5621_write(codec, RT5621_GPIO_OUTPUT_PIN_CTRL, 0x0000);//set GPIO output low level for mute speaker AMP
...
static int rt5621_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    dev = &i2c->dev;
    sysfs_create_group(&dev->kobj, &rt5621_attr_group);
...
static int rt5621_i2c_shutdown(struct i2c_client *client)
{
    struct snd_soc_codec *codec = i2c_get_clientdata(client);
    printk("##enter %s \n", __FUNCTION__);
    rt5621_write_mask(codec,RT5621_GPIO_OUTPUT_PIN_CTRL ,0x0000,0x0002);//Power off AMP
...
static int rt5621_probe(struct platform_device *pdev)
{
    ...
    if (codec == NULL)
    {
        pr_err("kzalloc for codec error\n");
        return -ENOMEM;
    }
    ...
    if (rt5621 == NULL)
    {
        pr_err("kzalloc for rt5621 error\n");
        kfree(codec);
        return -ENOMEM;
    }
    ...
    if (ret != 0)
    {
        pr_err("add rt5621 i2c driver error\n");
