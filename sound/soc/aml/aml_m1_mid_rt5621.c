                            ...
                            struct snd_pcm_hw_params *params)
{
    ...
    int ret;//realtek - 20110620
    // TODO
    printk("***Entered %s:%s rate:%d\n", __FILE__,__func__, params_rate(params));
    ...
    ret = snd_soc_dai_set_sysclk(codec_dai, 0, (256 * params_rate(params)), SND_SOC_CLOCK_OUT);
    if(ret) {
        printk(KERN_ERR "%s set snd soc codec dai sysclk failed: %d\n", __func__, ret);
        return ret;
    }
    //realtek - 20110620 [e]
...
static void rt5621_hp_detect_queue(struct work_struct* work)
{
    ...
    if(level == 0x1 && level != hp_detect_flag){ // HP
...
static void Init_Aux_Hp(struct snd_soc_codec *codec)
{
    if (hp_detect_flag==1){
...
static int aml_m1_codec_init(struct snd_soc_codec *codec)
{
    ...
    struct snd_soc_dai *dai = &codec->dai[0];
    int err;
    
    // set dai sysclk
    err = snd_soc_dai_set_sysclk(dai, 0, MCLK_RATE, SND_SOC_CLOCK_IN);
    if(err) {
        printk(KERN_ERR "%s set snd soc codec dai sysclk(%d) failed: %d\n", __func__, MCLK_RATE, err);
        return err;
    }    
