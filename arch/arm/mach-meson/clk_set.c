...
{
	int n = 1;
        int clk=READ_CBUS_REG_BITS(HHI_ETH_CLK_CNTL,0,32);
...
int sys_clkpll_setting(unsigned crystal_freq,unsigned  out_freq)
{
    ...
    if (READ_MPEG_REG(HHI_SYS_PLL_CNTL)!=pll_setting[(out_M-200)/100]){
        local_irq_save(flags);
        WRITE_MPEG_REG(HHI_SYS_PLL_CNTL, pll_setting[(out_M-200)/100]); // system PLL
