...
static struct aml_camera_i2c_fig_s gt2005_custom_init_script[] = {
	{0x0101,0x02}, // h_v
	{0xffff,0xff},
};
aml_plat_cam_data_t video_gt2005_data = {
	...
	.custom_init_script = gt2005_custom_init_script,
};
#endif
...
{
    if(is_ac_connected()){ //AC in after power off press
        arm_pm_restart("","charging_reboot");//kernel_restart("reboot");
    }
    set_gpio_val(GPIOA_bank_bit(8), GPIOA_bit_bit0_14(8), 0);
    set_gpio_mode(GPIOA_bank_bit(8), GPIOA_bit_bit0_14(8), GPIO_OUTPUT_MODE);
...
static void __init device_pinmux_init(void )
{
	...
	//weizaiyue marked for old hardware PTT-726
	#ifdef CONFIG_SIX_AXIS_SENSOR_MPU3050
    mpu3050_init_irq();
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0308
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_3,(1<<13));
#endif
}
