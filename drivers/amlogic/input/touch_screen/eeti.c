	...
	if( bNeedReport || (ContactID <= LastUpdateID) )
	{
		for(i=0; i<MAX_SUPPORT_POINT; i++)
		{
			if(PointBuf[i].Status >= 0)
			{
				...
				input_report_key(input_dev, BTN_TOUCH, PointBuf[i].Status);//weizaiyue
				...
				input_report_abs(input_dev, ABS_MT_WIDTH_MAJOR, 0);//weizaiyue
...
static struct input_dev * allocate_Input_Dev(void)
{
    ...
    set_bit(EV_KEY, pInputDev->evbit);//weizaiyue
    set_bit(BTN_TOUCH, pInputDev->keybit);//weizaiyue
