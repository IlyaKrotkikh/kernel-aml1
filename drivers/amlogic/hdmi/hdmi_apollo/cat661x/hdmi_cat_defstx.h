#ifndef _HDMI_CAT6611_DRV_H_
#define _HDMI_CAT6611_DRV_H_
...
#define B_ENTEST    (1<<7)
#define B_REF_RST (1<<5)	//Software RCLK reset.   
#define B_AREF_RST (1<<4)	//Software Audio clock    base signal reset.
#define B_VID_RST (1<<3)	//Software Video clock base  signal reset.
#define B_AUD_RST (1<<2)	//Audio FIFO reset.  
#define B_HDMI_RST (1<<1)
#define B_HDCP_RST (1<<0)	//HDCP reset.     
...
#define B_TXVIDSTABLE   (1<<4)	//it is related to the CLK/DE/HSYNC/VSYNC
    // read/write
...
#define B_LISTFAIL  (1<<1)	//Write this bit when process KSVList Check interrupt Routine. 1: HDCP Authentication FSM will return to wait and try state.
#define B_LISTDONE  (1<<0)	//Write this bit when process KSVList Check interrupt routine. Write this ¡®1¡¯ after PC check KSV FIFO list.
...
#define CAT_Switch_HDMITX_Bank(device,bank) \
    WriteByteHDMITX_CAT(REG_BANK_CTRL, bank == 1 ? B_BANK1 : B_BANK0) 
#endif				// _HDMI_CAT6611_DRV_H_
