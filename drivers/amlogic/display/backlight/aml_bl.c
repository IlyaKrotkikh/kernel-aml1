...
//#define AML_BL_DBG	/* debug */
#define AML_BL_CMP	/* Dynamic range compress. Good for auto brigthes level */
...
static int aml_bl_update_status(struct backlight_device *bd)
{
	...
	/* this is correction dynamic range compression
    	    for comfort use at digma TN matrix
        */
	brightness = ((brightness>>2)+74);
    }

    /* five steps hole to maximum */
    if( brightness > 250 )
        brightness = 255;
#endif
