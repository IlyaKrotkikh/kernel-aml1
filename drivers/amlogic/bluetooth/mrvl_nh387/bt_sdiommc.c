...
sd_card_to_host(bt_private * priv)
{
	...
	//heming
    //printk("Bluetooth sd_card_to_host: func->num=0x%4.04X device=0x%4.04X ioport=%d blksz=%d buf_block_len * blksz=%d\n",
    //      card->func->num, card->func->device, priv->bt_dev.ioport, blksz, buf_block_len * blksz);
...
#define MAX_WRITE_IOMEM_RETRY	2
	...
	//heming
	//printk("Bluetooth sbi_host_to_card: func->num=0x%4.04X device=0x%4.04X ioport=%d blksz=%d buf_block_len * blksz=%d\n",
		//	  card->func->num, card->func->device, priv->bt_dev.ioport, blksz, buf_block_len * blksz);
        ...
        if (ret < 0) {
            ...
            DBG_HEXDUMP(DAT_D, "SDIO Blk Wr", payload, nb);
            PRINTM(DATA, "SDIO Blk Wr dev%d: len=%d\n", hdev->id, nb);
        }
...
sbi_get_int_status(bt_private * priv, u8 * ireg)
{
    ...
    *ireg = 0;
    ...
    OS_INT_RESTORE;
    sdio_claim_host(card->func);
    PRINTM(INTR, "get_int_status dev%d: sdio_ireg=0x%x\n", hdev->id, sdio_ireg);
