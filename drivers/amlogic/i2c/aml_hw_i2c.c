/*
 * linux/drivers/amlogic/i2c/aml_hw_i2c.c
 */
							...
							int num)
{
	...
	if (p == NULL || p->len == 0) {
	    printk("I2C ERROR - NO DATA PRESENT.");
	...
	if (ret == 0){
		...
		if(i2c->i2c_debug != 0) {
		    if (printk_ratelimit())
			dev_err(&i2c_adap->dev, "[aml_i2c_xfer] error ret = %d (%s) token %d, " "master %s %dK addr 0x%x\n",
			ret, ret == -EIO ? "-EIO" : "-ETIMEOUT", i2c->cur_token, i2c->master_no?"a":"b", i2c->master_i2c_speed/1000,
			i2c->cur_slave_addr);
		}
	        mutex_unlock(&i2c->lock);
		return -EAGAIN;
							...
							int num)
{
	...
	if (ret == 0){
		...
		if(i2c->i2c_debug != 0) {
		    if (printk_ratelimit())
			dev_err(&i2c_adap->dev, "[aml_i2c_xfer_s2] error ret = %d (%s) token %d\t" "master %s %dK addr 0x%x\n", 
			ret, ret == -EIO ? "-EIO" : "-ETIMEOUT", i2c->cur_token, i2c->master_no?"a":"b", i2c->master_i2c_speed2/1000,
			i2c->cur_slave_addr);
		}
		mutex_unlock(&i2c->lock);
		return -EAGAIN;
