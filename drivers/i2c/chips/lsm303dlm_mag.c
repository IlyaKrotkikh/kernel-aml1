...
********************************************************************************
 Revision 1.0.1: 14/02/2011
  uses input polled device;
 Revision 1.0.1.1: 19/07/2011 by morris chen
  add the odr edge
*******************************************************************************/
...
#define LSM303DLM_MAG_ODR_75		0x00	/* 0.75Hz output data rate */
#define LSM303DLM_MAG_ODR1_5		0x04	/* 1.5Hz output data rate */
#define LSM303DLM_MAG_ODR3_0		0x08	/* 3Hz output data rate */
#define LSM303DLM_MAG_ODR7_5		0x0C	/* 7.5Hz output data rate */
#define LSM303DLM_MAG_ODR15		0x10	/* 15Hz output data rate */
#define LSM303DLM_MAG_ODR30		0x14	/* 30Hz output data rate */
#define LSM303DLM_MAG_ODR75		0x18	/* 75Hz output data rate */
...
static const struct output_rate odr_table[] = {
  {	1,	LSM303DLM_MAG_ODR75},
	{	13,	LSM303DLM_MAG_ODR75},
	{	34,	LSM303DLM_MAG_ODR30},
	{	67,	LSM303DLM_MAG_ODR15},
	{	134,	LSM303DLM_MAG_ODR7_5},
	{	334,	LSM303DLM_MAG_ODR3_0},
	{	667,	LSM303DLM_MAG_ODR1_5},
	{	1334,	LSM303DLM_MAG_ODR_75},
};
...
int lsm303dlm_mag_update_h_range(struct lsm303dlm_mag_data *mag,
				 u8 new_h_range)
{
	int err = -1;
	u8 buf[2];
	...
	if (atomic_read(&mag->enabled)) {
		...
		buf[0] = CRB_REG_M;
		buf[1] = new_h_range;
		err = lsm303dlm_mag_i2c_write(mag, buf, 1);
		if (err < 0)
			return err;
	}
...
int lsm303dlm_mag_update_odr(struct lsm303dlm_mag_data *mag, int poll_interval)
{
	int err = -1;
	int i;
	u8 config[2];
	...
	if (atomic_read(&mag->enabled)) {
		config[0] = CRA_REG_M;
		err = lsm303dlm_mag_i2c_write(mag, config, 1);
		if (err < 0)
			return err;
	}
...
static int lsm303dlm_mag_get_data(struct lsm303dlm_mag_data *mag,
					       int *xyz)
{
	int err = -1;
...
static void lsm303dlm_mag_report_values(struct lsm303dlm_mag_data *mag,
					int *xyz)
{
...
static int lsm303dlm_mag_hw_init(struct lsm303dlm_mag_data *mag)
{
	...
	err = lsm303dlm_mag_i2c_write(mag, buf, 3);
...
static void lsm303dlm_mag_device_power_off(struct lsm303dlm_mag_data *mag)
{
	int err;
...
static int lsm303dlm_mag_device_power_on(struct lsm303dlm_mag_data *mag)
{
	int err;
	...
	if (!mag->hw_initialized) {
		...
		if (err < 0) {
			lsm303dlm_mag_device_power_off(mag);
			return err;
		}
	} else {
		err = lsm303dlm_mag_i2c_write(mag, buf, 1);
...
static int lsm303dlm_mag_enable(struct lsm303dlm_mag_data *mag)
{
	...
	if (!atomic_cmpxchg(&mag->enabled, 0, 1)) {
		...
		err = lsm303dlm_mag_device_power_on(mag);
		if (err < 0) {
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	printk("%s, h_range = %d", __func__, val);
	switch (val) {
	case LSM303DLM_MAG_H_1_3G:
		range = 1300;
		break;
			      ...
			      const char *buf, size_t size)
{
	...
	lsm303dlm_mag_update_h_range(mag, val);
	mutex_unlock(&mag->lock);
	return size;
}
				...
				const char *buf, size_t size)
{
	...
	mutex_unlock(&mag->lock);
	x[1] = val;
	rc = lsm303dlm_mag_i2c_write(mag, x, 1);
	return size;
}
				...
				char *buf)
{
	...
	data = mag->reg_addr;
	mutex_unlock(&mag->lock);
	rc = lsm303dlm_mag_i2c_read(mag, &data, 1);
...
static ssize_t attr_addr_set(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct lsm303dlm_mag_data *gyro = dev_get_drvdata(dev);
	unsigned long val;
...
static void lsm303dlm_mag_input_poll_func(struct input_polled_dev *dev)
{
	struct lsm303dlm_mag_data *mag = dev->private;
	...
	mutex_lock(&mag->lock);
	err = lsm303dlm_mag_get_data(mag, xyz);
	if (err < 0)
		dev_err(&mag->client->dev, "get_magnetometer_data failed\n");
	else
		lsm303dlm_mag_report_values(mag, xyz);

	mutex_unlock(&mag->lock);
}

int lsm303dlm_mag_input_open(struct input_dev *input)
{
	struct lsm303dlm_mag_data *mag = input_get_drvdata(input);

	return lsm303dlm_mag_enable(mag);
}

void lsm303dlm_mag_input_close(struct input_dev *dev)
{
	struct lsm303dlm_mag_data *mag = input_get_drvdata(dev);

	lsm303dlm_mag_disable(mag);
}

static int lsm303dlm_mag_validate_pdata(struct lsm303dlm_mag_data *mag)
{
...
static int lsm303dlm_mag_input_init(struct lsm303dlm_mag_data *mag)
{
	int err = -1;
	...
	mag->input_poll_dev->poll = lsm303dlm_mag_input_poll_func;
	mag->input_poll_dev->poll_interval = mag->pdata->poll_interval;

	input = mag->input_poll_dev->input;

	input->open = lsm303dlm_mag_input_open;
	input->close = lsm303dlm_mag_input_close;
	...
	input->name = LSM303DLM_MAG_DEV_NAME;
...
static void lsm303dlm_mag_input_cleanup(struct lsm303dlm_mag_data *mag)
{
...
{
	struct lsm303dlm_mag_data *mag;

	int err = -1;

	pr_err("%s: probe start\n", LSM303DLM_MAG_DEV_NAME);
	...
	mag = kmalloc(sizeof(*mag), GFP_KERNEL);
	if (mag == NULL) {
		dev_err(&client->dev,
	...
	err = lsm303dlm_mag_validate_pdata(mag);
	if (err < 0) {
	...
	err = lsm303dlm_mag_update_h_range(mag, mag->pdata->h_range);
	if (err < 0) {
	...
	err = lsm303dlm_mag_input_init(mag);
	if (err < 0)
		goto err3;
...
err4:
	lsm303dlm_mag_input_cleanup(mag);
err3:
	lsm303dlm_mag_device_power_off(mag);
err2:
	if (mag->pdata->exit)
...
static int lsm303dlm_mag_remove(struct i2c_client *client)
{
	struct lsm303dlm_mag_data *mag = i2c_get_clientdata(client);
	#ifdef DEBUG
	pr_info(KERN_INFO "LSM303DLM driver removing\n");
	#endif
	lsm303dlm_mag_input_cleanup(mag);
	lsm303dlm_mag_device_power_off(mag);
...
static int lsm303dlm_mag_suspend(struct device *dev)
{
	#ifdef CONFIG_SUSPEND
	struct i2c_client *client = to_i2c_client(dev);
	struct lsm303dlm_data *gyro = i2c_get_clientdata(client);
	#if DEBUG
	pr_info(KERN_INFO "lsm303dlm_suspend\n");
	#endif
	/* TO DO */
	#endif
	return 0;
...
static int lsm303dlm_mag_resume(struct device *dev)
{
	#ifdef CONFIG_SUSPEND
	struct i2c_client *client = to_i2c_client(dev);
	struct lsm303dlm_data *mag = i2c_get_clientdata(client);
	#if DEBUG
	pr_info(KERN_INFO "lsm303dlm_resume\n");
	#endif
	/* TO DO */
	#endif
	return 0;
...
static const struct i2c_device_id lsm303dlm_mag_id[] = {
	{LSM303DLM_MAG_DEV_NAME, 0},
	{},
};

MODULE_DEVICE_TABLE(i2c, lsm303dlm_mag_id);

static struct dev_pm_ops lsm303dlm_pm = {
	.suspend = lsm303dlm_mag_suspend,
	.resume = lsm303dlm_mag_resume,
};

static struct i2c_driver lsm303dlm_mag_driver = {
	.driver = {
			.owner = THIS_MODULE,
			.name = LSM303DLM_MAG_DEV_NAME,
			.pm = &lsm303dlm_pm,
		   },
...
{
	pr_info(KERN_INFO "lsm303dlm magnetometer driver\n");
...
MODULE_DESCRIPTION("lsm303dlm driver for the magnetometer section");
