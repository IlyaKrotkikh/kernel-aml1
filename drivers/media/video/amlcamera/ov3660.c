...
static int ov3660_v_active=600;
...
/******if use 50HZ banding remove, refer to below setting********/
	...
	{0x3a09, 0x75}, //50HZ BAND WIDTH
	{0x3a0e, 0x07}, //50HZ MAX BAND
...
*
*************************************************************************/
void OV3660_set_param_brightness(struct ov3660_device *dev, int value)//ÃÃÂ¶Ã
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
	int temp;
...
*
*************************************************************************/
void OV3660_set_param_contrast(struct ov3660_device *dev, int value)//ÃÃÂ¶Ã
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
...
*
*************************************************************************/
void OV3660_set_param_saturation(struct ov3660_device *dev, int value)//Â±Â¥ÂºÃÂ¶Ã
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
...
*
*************************************************************************/
void OV3660_set_param_wb(struct ov3660_device *dev,enum  camera_wb_flip_e para)//Â°ÃÃÂ½ÂºÃ¢
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
...
*
*************************************************************************/
void OV3660_set_param_exposure(struct ov3660_device *dev,enum camera_exposure_e para)//ÃÃÂ¹Ã¢ÂµÃ·Â½Ã
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
...
*
*************************************************************************/
void OV3660_set_param_effect(struct ov3660_device *dev,enum camera_effect_flip_e para)//ÃÃÃÂ§ÃÃ¨ÃÃ
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
	unsigned char temp;
...
static struct aml_camera_i2c_fig_s pic1600x1200[]={
	...
	{0x3821,0x04},	// Horizontal binning off
	{0x3708,0x63},
	{0x3709,0x12},
	{0x370c,0x0c},
	{0x4520,0xb0},
	//{0x3821,0x00},	// Mirror on,binning off
	{0x4514,0xbb},
	{0x303a,0x00},
	{0x303b,0x1b},
	{0x303c,0x13},
...
void OV3660_set_resolution(struct ov3660_device *dev,int height,int width)
{
	...
	if((width<1600)&&(height<1200)){
		...
		mdelay(20);
		ov3660_h_active=1600;
		ov3660_v_active=1200;
	} else if (width>=2000&&height>=1500) {
		//2048x1536
		//pic_set_size(dev, pic1600x1200);
		pic_set_size(dev, pic2048x1536);
		mdelay(20);
		ov3660_h_active=2032;
		ov3660_v_active=1536;
	}	
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
    ...
    para.fmt_info.fmt = TVIN_SIG_FMT_MAX+1;//TVIN_SIG_FMT_CAMERA_1280X720P_30Hz;
	para.fmt_info.frame_rate = 236;
	para.fmt_info.h_active = ov3660_h_active;
	para.fmt_info.v_active = ov3660_v_active;
#ifdef CONFIG_ARCH_MESON3
	para.fmt_info.hsync_phase = 1;//0x1
	para.fmt_info.vsync_phase  = 0;//0x1
#endif
...
//    TVIN_SIG_FMT_CAMERA_1280X720P_30Hz,
	...
	videobuf_queue_vmalloc_init(&fh->vb_vidq, &ov3660_video_qops,
			...
			sizeof(struct ov3660_buffer), fh);
...
static int ov3660_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *chip)
{
    ...
    printk("ov3660_g_chip_ident\n");
	return 0;//v4l2_chip_ident_i2c_client(client, chip, V4L2_IDENT_OV3660, 0);
}
