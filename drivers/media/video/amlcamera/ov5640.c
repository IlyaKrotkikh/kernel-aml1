...
static int ov5640_h_active=478;
static int ov5640_v_active=640;
...
static struct v4l2_frmsize_discrete ov5640_prev_resolution[]=
{
    {  320,  240 },    /* QVGA */
    {  640,  480 },        /* VGA */
//    { 720,  1280 },    /* SXGA */
...
static struct v4l2_frmsize_discrete ov5640_pic_resolution[]=
{
    //{  320,  240 },    /* QVGA */
    //{  640,  480 },        /* VGA */
    //{ 1280,  720 },    /* SXGA */
...
struct aml_camera_i2c_fig_s OV5640_capture_script[] = {
    {0x3035, 0x31},
    {0x3036, 0x69},
    {0x3c07, 0x07},
...
void OV5640_init_preview(struct ov5640_device *dev)
{
    struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
    int i=0;
    printk("OV5640_init_preview!!!!!!!\n");
    while(1)
    {
        if (OV5640_preview_script[i].val==0xff&&OV5640_preview_script[i].addr==0xffff) 
        {
            printk("OV5640_write_regs success in OV5640_init_preview.\n");
            break;
        }
        if((i2c_put_byte(client,OV5640_preview_script[i].addr, OV5640_preview_script[i].val)) < 0)
        {
            printk("fail in initial OV5640 preview. i=%d\n",i);
        	break;
        }
        	i++;
    }
...
void OV5640_init_capture(struct ov5640_device *dev)
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
    int i=0;
    while(1)
    {
        if (OV5640_capture_script[i].val==0xff&&OV5640_capture_script[i].addr==0xffff) 
        {
        	printk("OV5640_write_regs success in OV5640_init_capture\n");
        	break;
        }
        if((i2c_put_byte(client,OV5640_capture_script[i].addr, OV5640_capture_script[i].val)) < 0)
        {
        	printk("fail in initial OV5640_capture_script. i=%d\n",i);
    	break;
        }
    	i++;
    }
...
void OV5640_init_regs(struct ov5640_device *dev)
{
    ...
    while(1)
    {
        ...
        {
        	printk("OV5640_write_regs success in initial OV5640.\n");
        	break;
        }
        if((i2c_put_byte(client,OV5640_script[i].addr, OV5640_script[i].val)) < 0)
        {
        	printk("fail in initial OV5640. i=%d\n",i);
    	    break;
        }
    	i++;
    }
...
*
*************************************************************************/
void OV5640_set_param_wb(struct ov5640_device *dev,enum  camera_wb_flip_e para)//Â°ÃÃÂ½ÂºÃ¢
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
...
*
*************************************************************************/
void OV5640_set_param_exposure(struct ov5640_device *dev,enum camera_exposure_e para)//ÃÃÂ¹Ã¢ÂµÃ·Â½Ã
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
    ...
    switch (para)
    {
    	case EXPOSURE_N4_STEP:  //Â¸Âº4ÂµÂµ  
    	...
    	case EXPOSURE_0_STEP://ÃÂ¬ÃÃÃÃ£ÂµÂµ
    	...
    	case EXPOSURE_P1_STEP://ÃÃ½ÃÂ»ÂµÂµ
...
*
*************************************************************************/
void OV5640_set_param_effect(struct ov5640_device *dev,enum camera_effect_flip_e para)//ÃÃÃÂ§ÃÃ¨ÃÃ
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
  
    switch (para)
    {
    	case CAM_EFFECT_ENC_NORMAL://ÃÃ½Â³Â£
        	i2c_put_byte(client,0x5001,0x03);//disable effect
        	break;        

    	case CAM_EFFECT_ENC_GRAYSCALE://Â»ÃÂ½Ã
        	i2c_put_byte(client,0x5001,0x83);
        	i2c_put_byte(client,0x5580,0x20);
        	break;

    	case CAM_EFFECT_ENC_SEPIA://Â¸Â´Â¹Ã
                 /*i2c_put_byte(client,0x0115,0x0a);
        	i2c_put_byte(client,0x026e,0x60);
        	i2c_put_byte(client,0x026f,0xa0);*/
        	break;        
                
    	case CAM_EFFECT_ENC_SEPIAGREEN://Â¸Â´Â¹ÃÃÃ
            /*i2c_put_byte(client,0x0115,0x0a);
        	i2c_put_byte(client,0x026e,0x20);
        	i2c_put_byte(client,0x026f,0x00);*/
        	break;                    

    	case CAM_EFFECT_ENC_SEPIABLUE://Â¸Â´Â¹ÃÃÂ¶
            /*i2c_put_byte(client,0x0115,0x0a);
        	i2c_put_byte(client,0x026e,0xfb);
        	i2c_put_byte(client,0x026f,0x00);*/
        	break;                                

    	case CAM_EFFECT_ENC_COLORINV://ÂµÃÃÂ¬
        	i2c_put_byte(client,0x5001,0x83);
...
*
*************************************************************************/
void OV5640_NightMode(struct ov5640_device *dev,enum  camera_night_mode_flip_e enable)
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
...
void OV5640_set_resolution(struct ov5640_device *dev,int height,int width)
{
#if 0
    printk("OV5640_set_resolution height =%d,width=%d",height,width);
    int ret;
    struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
    if(!vidio_set_fmt_ticks){
        //OV5640_init_preview(dev);
        ov5640_h_active=480;
        ov5640_v_active=640;
    }
    else{ 
        //OV5640_init_capture(dev);
        msleep(10);// 10
        ov5640_h_active=1944;
        ov5640_v_active=2592;
        //ov5640_h_active=1200;
        //ov5640_v_active=1600;
    }
#endif
    printk("OV5640_set_resolution height: %d; width: %d \n");
    #if 1	         
    ...
    else if((height == 1200)&&(width == 1600))
    {
        ...
        #endif
        msleep(10);// 10
        ov5640_h_active=1600;
        ov5640_v_active=1200;
    }
    else if((height == 1536)&&(width == 2048))
    {
        ...
        #endif
        msleep(10);// 10
        ov5640_h_active=2048;
        ov5640_v_active=1536;
    }
    else if((height == 1944)&&(width == 2592))
    {
        ...
        #endif
        msleep(10);// 10
        ov5640_h_active=2592;
        ov5640_v_active=1936;
    }
    #else
                	...
                	struct v4l2_format *f)
{
    ...
    if(f->fmt.pix.pixelformat==V4L2_PIX_FMT_RGB24){
        ...
        OV5640_set_resolution(dev,fh->height,fh->width);
    }
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	para.fmt_info.h_active = ov5640_h_active;
	para.fmt_info.v_active = ov5640_v_active;
...
static int vidioc_enum_framesizes(struct file *file, void *fh,struct v4l2_frmsizeenum *fsize)
{
	...
	printk("ov5640_prev_resolution[fsize->index]   before fsize->index== %d\n",fsize->index);//potti
    	if (fsize->index >= ARRAY_SIZE(ov5640_prev_resolution))
        	return -EINVAL;
    	frmsize = &ov5640_prev_resolution[fsize->index];
	...
	printk("ov5640_pic_resolution[fsize->index]   before fsize->index== %d\n",fsize->index);
    	if (fsize->index >= ARRAY_SIZE(ov5640_pic_resolution))
        	return -EINVAL;
    	frmsize = &ov5640_pic_resolution[fsize->index];
