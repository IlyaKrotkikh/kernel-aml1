...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	para.fmt_info.h_active = ov2655_h_active;
	para.fmt_info.v_active = ov2655_v_active;
...
//    TVIN_SIG_FMT_CAMERA_1280X720P_30Hz,
	...
	videobuf_queue_vmalloc_init(&fh->vb_vidq, &ov2655_video_qops,
			...
			sizeof(struct ov2655_buffer), fh);
			...
			const struct i2c_device_id *id)
{
	...
	if (plat_dat) {
		...
		if(t->platform_dev_data.device_probe) {
			t->platform_dev_data.device_probe();
